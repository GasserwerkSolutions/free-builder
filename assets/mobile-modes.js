...
export function initMobileModes(context) {
    ...
    const onMediaChange = () => { applyMobileMode(context); if (!isMobileModeActive())
    ...
    const onFocusIn = (event) => { if (isEditableControl(event.target))
        scheduleKeyboardCheck(); };
    const onFocusOut = () => scheduleKeyboardCheck();
    const onViewportResize = () => refreshModeBarForKeyboard();
    baselineInnerHeight = window.innerHeight;
...
export function setMobileMode(context, mode, spoken = true) {
    if (context.mobileMode === mode) {
        applyMobileMode(context);
        return;
    }
    if (context.mobileMode === "edit")
    ...
    applyMobileMode(context);
    if (isMobileModeActive())
        window.scrollTo(0, mode === "edit" ? context.mobileEditorScroll : 0);
    if (spoken)
        announce(context, mode === "edit" ? "Der Bearbeitungsmodus ist aktiv." : "Die Vorschau ist aktiv. Tippe auf einen Inhalt, um ihn zu bearbeiten.");
}
/** A jump into a field is worthless while the preview covers it. */
export function ensureMobileEditMode(context) {
    if (isMobileModeActive() && context.mobileMode !== "edit")
...
export function isSectionSheetOpen() {
...
export function openSectionSheet(context) {
    ...
    const preferred = sheet.querySelector(".section-sheet__entry.is-active") ?? sheet.querySelector(".section-sheet__entry");
    preferred?.focus();
}
...
export function refreshModeBarForKeyboard() {
    document.querySelector(".mode-switch")?.classList.toggle("is-keyboard-hidden", isMobileModeActive() && keyboardLikelyOpen());
}
...
function renderSectionSheet() {
    ...
    if (!list)
        return;
    list.textContent = "";
    document.querySelectorAll(".surface-nav [data-panel-target]").forEach((navButton, index) => {
...
function setSheetBackgroundInert(context, on) {
    [document.querySelector(".topbar"), context.workspace, document.querySelector(".mode-switch")]
...
function maybeShowMobileHint() {
    if (!isMobileModeActive() || readPreference(MOBILE_HINT_KEY))
        return;
