...
const TEAM_PANEL_TARGET: PreviewTarget = { kind: "panel", panel: "team" };
const STAFF_TARGET_FIELDS: readonly string[] = ["name", "role", "bio"];
...
function staffTarget(staffClientId: string, field: string): PreviewTarget {
  return STAFF_TARGET_FIELDS.includes(field)
    ? { kind: "staff", staffClientId, field: field as "name" | "role" | "bio" }
    : TEAM_PANEL_TARGET;
}
...
export function renderTeam(store: BuilderStore): void {
  ...
  list.querySelectorAll<HTMLElement>("[data-staff-card]").forEach((card, index) => {
...
export function installTeamUi(store: BuilderStore, repository: DraftRepository): void {
  ...
  store.subscribe((_draft, mutation) => {
    ...
    if (rebuilt || shapeFingerprint(store) !== lastShapeFingerprint) renderTeam(store);
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "no-services") mutateStaffServices(store, staffId, (draft) => setAllBookableServicesForStaff(draft, staffId, false), "Leistungszuordnung geleert");
  ...
  const handleField = (event: Event): void => {
    ...
    if (!staffId) return;
    const field = target.dataset.staffField as StaffEditableField | undefined;
    if (field) {
