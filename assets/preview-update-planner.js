import { MAX_TESTIMONIALS, getAtPath } from "./domain.js";
...
const RENDERED_STAFF_FIELDS = new Set(["name", "role", "bio", "active"]);
function isStaffVisible(person) { return person.active && Boolean(person.name.trim()); }
function visibleStaffCount(draft) { return draft.staff.filter(isStaffVisible).length; }
function isTestimonialFilled(item) { return Boolean(item.quote.trim() && item.name.trim()); }
function filledTestimonialCount(draft) {
    return draft.testimonials.items.filter(isTestimonialFilled).slice(0, MAX_TESTIMONIALS).length;
}
function visibleTestimonialCount(draft) {
...
function staffShowsService(draft, serviceClientId) {
 ...
 */
function presenceFlipped(before, after) { return (before > 0) !== (after > 0); }
        ...
        switch (effect.type) {
            ...
            case "field-set": {
                ...
                if (effect.field === "testimonials.enabled") {
                    ...
                    if (filledTestimonialCount(draft) > 0)
            ...
            case "service-field-set": {
                ...
                if (effect.field === "name" && staffShowsService(draft, effect.serviceClientId))
                    regions.add("team");
                break;
            }
            ...
            case "collection-move": {
                ...
                if (outcome)
                    return { kind: "full", revision, reason: outcome };
                break;
            }
            ...
            case "staff-services-set": {
                ...
                if (person && isStaffVisible(person))
                    regions.add("team");
                break;
            }
 ...
 */
function collectCollectionChange(effect, draft, regions) {
    if (effect.collection === "services") {
        regions.add("services");
        if (effect.type === "collection-move")
            return null;
        if (effect.type === "collection-insert") {
            if (staffShowsService(draft, effect.clientId))
                regions.add("team");
            return null;
        }
    ...
    if (effect.collection === "staff") {
        ...
        if (effect.type === "collection-insert") {
            ...
            if (!person || !isStaffVisible(person))
                return null;
            if (presenceFlipped(countAfter - 1, countAfter))
                return "layout";
            regions.add("team");
            return null;
        }
        ...
        if (person && isStaffVisible(person) && countAfter > 1)
            regions.add("team");
        return null;
    }
    ...
    if (item && draft.testimonials.enabled && isTestimonialFilled(item) && countAfter > 1)
        regions.add("voices");
    return null;
}
function collectStaffField(effect, draft, regions) {
    const person = draft.staff.find((item) => item.clientId === effect.staffClientId);
    if (!person)
        return "unsupported";
    ...
    if (presenceFlipped(countBefore, countAfter))
        return "layout";
    if (RENDERED_STAFF_FIELDS.has(effect.field) && (visibleBefore || visibleAfter))
        regions.add("team");
    return null;
}
function collectTestimonialField(effect, draft, regions) {
...
function regionForTarget(target) {
    ...
    if (target.kind === "field")
        return TEXT_FIELDS[target.field] ?? null;
    return null;
}
