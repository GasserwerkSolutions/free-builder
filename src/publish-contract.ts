...
export type PublishOutcome =
  ...
  | {
    ...
    status: number | null;
    detail: string | null;
    /** What `Retry-After` said, in milliseconds. Null means the server did not say. */
 ...
 */
export function outcomeForResponse(status: number, payload: unknown, retryAfterMs: number | null = null): PublishOutcome {
  ...
  if (status === 429) return publishFailure("RATE_LIMITED", status, errorText(payload), retryAfterMs);
  if (status === 400) {
    const message = errorText(payload) ?? "";
    if (MALFORMED_HINT.test(message)) return publishFailure("MALFORMED_REQUEST", status, message || null);
