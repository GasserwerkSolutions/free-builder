...
import { announce } from "./ui-shared.js";
...
export function resolveEditorTarget(target) {
    if (target.kind === "field")
        return document.querySelector(`[data-bind="${cssEscape(target.field)}"]`);
    if (target.kind === "service")
        return document.querySelector(`[data-service-card][data-service-id="${cssEscape(target.serviceClientId)}"] [data-service-field="${target.field}"]`);
    if (target.kind === "testimonial")
        return document.querySelector(`[data-testimonial-card][data-testimonial-id="${cssEscape(target.testimonialClientId)}"] [data-testimonial-field="${target.field}"]`);
    if (target.kind === "staff")
        return document.querySelector(`[data-staff-card][data-staff-id="${cssEscape(target.staffClientId)}"] [data-staff-field="${target.field}"]`);
