...
import { PREVIEW_CHANNEL, PREVIEW_PROTOCOL_VERSION, isPreviewMessageEnvelope, parseNavigateMessage, parseReadyMessage, parseScrollMessage, parseUpdateResult, resolveParentOrigin, } from "./preview-contract.js";
...
export class PreviewRuntime {
    ...
    acceptReady(revision) {
        // A late "ready" from the document we already patched past would rewind the preview.
        if (this.ready || revision !== this.fullRenderRevision || revision < this.appliedRevisionValue)
            return true;
        this.clearReadyTimer();
        this.ready = true;
    ...
    acceptUpdateResult(requestId, revision, success) {
        const active = this.inFlight;
        if (!active || requestId !== active.requestId)
            return true;
        clearTimeout(active.timeout);
        this.inFlight = null;
        if (!success || revision !== active.revision) {
            this.startFullRender(1);
            return true;
        }
        this.appliedRevisionValue = revision;
        this.processedRevisionValue = revision;
        if (this.desiredRevisionValue > this.processedRevisionValue || this.pending.length)
            this.scheduleFlush(0);
        return true;
    }
    scheduleFlush(delay) {
        if (this.destroyed || this.inFlight || this.coalesceTimer)
            return;
        ...
        if (!this.ready && !this.readyExhausted)
            return;
    ...
    flushPending() {
        ...
        if (!this.ready) {
            ...
            if (this.readyExhausted)
                this.startFullRender(0);
            return;
        }
