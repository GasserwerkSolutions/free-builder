import { getAtPath, type BuilderDraftV2 } from "./domain.js";
import type { DraftMutation, EditableFieldPath } from "./draft-mutations.js";
 ...
 */
function hasTeamRegion(draft: Readonly<BuilderDraftV2>): boolean {
  ...
  for (const mutation of mutations) {
    ...
    switch (effect.type) {
      ...
      case "collection-move": {
        ...
        if (collection === "services") { regions.add("services"); if (hasTeamRegion(draft)) regions.add("team"); }
        else if (collection === "staff") regions.add("team");
        else regions.add("voices");
        break;
      }
      case "testimonial-field-set": regions.add("voices"); break;
      case "staff-field-set":
      case "staff-services-set": regions.add("team"); break;
